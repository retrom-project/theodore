...
def source_paths() -> list[bytes]:
    def visit(repository: Path, prefix: bytes) -> set[bytes]:
        ...
        for raw in result.stdout.split(b"\0"):
            if not raw:
                continue
            target = repository / os.fsdecode(raw)
            if not os.path.lexists(target):
                continue
            if target.is_dir():
                if not (target / ".git").exists():
                    raise SystemExit("PFB_WORKTREE_INVALID")
                paths.update(visit(target, prefix + raw + b"/"))
            else:
                paths.add(prefix + raw)
        return paths

    return sorted(visit(ROOT, b""))
